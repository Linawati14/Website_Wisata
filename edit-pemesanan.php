...
<head>
    ...
    $query_mysql = mysqli_query($connect, "SELECT * FROM tb_pemesanan WHERE id_pemesanan='$id'") or die(mysqli_error($connect));
    $nomor = 1;
    ?>
</head>
        ...
        <div class="container">
            <div class="row justify-content-center pb-4">
                <div class="col-md-7 text-center heading-section ftco-animate">
                    <h2 class="mb-4">Menu Pembayaran Pesanan</h2>
                </div>
            </div>
            <?php while ($data = mysqli_fetch_array($query_mysql)) { ?>
                <form role="form" action="edit-aksi-pemesanan.php" method="post" enctype="multipart/form-data">
                    <label>Total Pembayaran :</label>
                    <div class="form-inline">
                        <div class="form-group">
                            <input type="hidden" name="id" value="<?php echo $data['id_pemesanan'] ?>">
                            <input type="text" name="total_pembayaran" value="<?php echo $data['total_pembayaran'] ?>" class="form-control">
                        </div>
                    </div><br>
                    <label>Uang Bayar :</label>
                    <div class="form-inline">
                        <div class="form-group">
                            <input type="number" name="bayar" id="bayar" value="<?php echo $data['bayar'] ?>" class="form-control">
                        </div>
                        <div class="form-group" style="margin-left: 7px; margin-right: 10px;">
                            <input type="number" name="kembali" id="kembali" class="form-control" value="<?php echo $data['kembali'] ?>" readonly>
                        </div>
                    </div><br>
                ...
                <script>
                    // Add event listener to 'bayar' input
                    document.getElementById('bayar').addEventListener('input', function() {
                        var totalPembayaran = parseFloat('<?php echo $data['total_pembayaran']; ?>');
                        var bayar = parseFloat(this.value);
                        var kembali = bayar - totalPembayaran;

                        // Update 'kembali' input value
                        document.getElementById('kembali').value = isNaN(kembali) ? 0 : kembali.toFixed(2);
                    });
                </script>
            <?php } ?>
