...
<body>
    ...
    <section class="ftco-section testimony-section bg-bottom" style="background-image: url(images/bg_3.jpg);">
        <div class="container">
            ...
            $query = "SELECT id_wisata, nama FROM tb_wisata";
            $result = $connect->query($query);
            $query2 = "SELECT id, id FROM tb_user";
            $result2 = $connect->query($query2);
            ?>
            <form role="form" action="tambah-aksi-pemesanan.php" method="post" enctype="multipart/form-data">
                <div class="form-inline">
                    <div class="form-group">
                        <!-- <input type="text" name="wisata" class="form-control" id="exampleInputEmail2" placeholder="wisata"> -->
                        <select name="wisata" class="form-control select2" id="exampleInputEmail2">
                            <?php
                            while ($row = $result->fetch_assoc()) {
                                echo '<option name="wisata" value="' . $row['nama'] . '">' . $row['nama'] . '</option>';
                            }
                            $result->free_result();
                            $connect->close();
                            ?>
                        </select>
                        <input hidden type="text" name="id_wisata" id="selected_wisata" readonly>
                        <input hidden type="text" value="1" name="id">
                    </div>
                </div><br>
                <div class="form-inline">
                    <div class="form-group">
                        <input type="datetime-local" name="tanggal_kunjungan" class="form-control" id="exampleInputEmail2" require>
                    </div>
                    <div class="form-group" style="margin-left: 7px;margin-right: 10px;">
                        <input type="number" name="jumlah_tiket" class="form-control" id="exampleInputPassword2" placeholder="Jumlah">
                    </div>
                </div><br>
                <div class="form-group">
                ...
                <div class="form-group">
                    <input type="text" class="form-control" name="total_pembayaran" id="exampleInputEmail1" placeholder="Total Pembayaran">
                </div>
                <!-- <div class="form-group">
    ...
    <script>
        // Saat opsi dipilih, perbarui nilai input teks
        $(document).ready(function() {
            $('#exampleInputEmail2').change(function() {
                var selectedWisata = $(this).val();
                $('#selected_wisata').val(selectedWisata);
            });
        });
    </script>
</body>
